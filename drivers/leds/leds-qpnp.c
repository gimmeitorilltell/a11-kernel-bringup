 ...
 *  @use_blink - use blink sysfs entry, for qcom lut breath blink not htc blink
 ...
 *  @cabc_en - selection of CABC for current sink
 */
...
/*
 * to_torch_enable(): what FBAD shall be torch mode
 */
...
/*
 * to_brightness_current(): convert flashlight brightness attribute definition into brightness current
 */
...
static int to_brightness_current(int brightness_level)
{
    ...
    if (val <= LED_HALF) {
        if (val == FBAD_OFF) {
            ...
            return 75;
        } else { /* all other values mapping to FBAD_TORCH */
            return 125;
        }
    } else {
        if (val == FBAD_PREFLASH) {
            ...
            return 700;
        } else { /* all other values mapping to FBAD_FULL */
            return 750;
        }
    }
...
{
    /*
     * brightness current = led->flash_cfg->current_prgm * 12.5mA
     */
	...
	int val = led->cdev.brightness;
        ...
        if (!strcmp(led->cdev.default_trigger, "htc_flash_mode")) {
            /* if trigger name is "htc_flash_mode", the brightness is brightness level. */
				...
				struct device_node *node)
{
	...
	/*use-blink is for qcom lut breath blink, not for htc blink*/
	if (pwm_cfg->mode == LPG_MODE || pwm_cfg->use_blink) {
		pwm_cfg->duty_cycles =
