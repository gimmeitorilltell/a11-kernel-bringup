...
static unsigned int gpio_33_index = 0;
	...
	for (index = 0; index < power_setting_array->size; index++) {
		...
		case SENSOR_CLK:
			...
			if (power_setting->config_val)
				s_ctrl->clk_info[power_setting->seq_val].
                    ...
                    CDBG("%s: %d clk_rate %ld \n", __func__, __LINE__, s_ctrl->clk_info[power_setting->seq_val].clk_rate);
		...
		case SENSOR_GPIO:
			...
			if ( data->gpio_conf->gpio_num_info->gpio_num[power_setting->seq_val] == 429)
			{
				gpio_429_index ++;
                    ...
                    if ( data->gpio_conf->gpio_num_info->gpio_num[power_setting->seq_val] == 33)
			{
				gpio_33_index ++;
			}
		...
		case SENSOR_VREG:
			...
			if (cam_vreg->type == REG_GPIO) {
				...
				printk("%s: %d cam_vreg_gpio %d \n", __func__, __LINE__, cam_vreg_gpio);
				...
				CDBG("%s: %d cam_vreg name %s \n", __func__, __LINE__, cam_vreg->reg_name);
...
int32_t msm_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	for (index = (power_setting_array->size - 1); index >= 0; index--) {
		...
		case SENSOR_GPIO:
			...
			if (data->gpio_conf->gpio_num_info->gpio_num[power_setting->seq_val] == 33 && gpio_33_index > 0)
			{
				gpio_33_index --;
				if (gpio_33_index != 0)
				{
